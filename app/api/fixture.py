...
@router.get("/")
async def read_fixtures(session: SessionDep, status: str | None = None) -> list[Fixture]:
    if status:
...
async def get_fixture(fixture_id: int, session: SessionDep):
...
@router.get("/next")
async def next_fixture(session: SessionDep):
...
@router.get("/form")
async def get_form(session: SessionDep, limit = 5) -> list[Fixture]:
